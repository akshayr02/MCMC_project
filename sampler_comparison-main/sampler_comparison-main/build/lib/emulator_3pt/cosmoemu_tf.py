...
# Adapted by Pierre Burger for general-purpose feature emulation
...
#               NN
# =================================
class cosmoemu_tf(tf.keras.Model):
    r"""
    ...
    def __init__(self, 
        ...
        """
        # super
        super(cosmoemu_tf, self).__init__()
        ...
        # else set variables from input arguments
            ...
            # architecture
            self.architecture = [self.n_parameters] + self.n_hidden + [self.feature_dimensions]
            self.n_layers = len(self.architecture) - 1

            # input parameters mean and std
            self.parameters_mean_ = np.zeros(self.n_parameters)
            self.parameters_std_ = np.ones(self.n_parameters)

            # feature mean and std
            self.features_mean_ = np.zeros(self.feature_dimensions)
            self.features_std_ = np.ones(self.feature_dimensions)
            
            self.scaling_subtraction_ = np.zeros(self.feature_dimensions)
            self.scaling_division_ = np.ones(self.feature_dimensions)
        ...
        self.scaling_subtraction = tf.constant(self.scaling_subtraction_, dtype=dtype, name='subtraction')
        self.scaling_division = tf.constant(self.scaling_division_, dtype=dtype, name='division')
        ...
        if self.verbose:
            multiline_str = "\nInitialized cosmoemu_tf model, \n" \
                            f"mapping {self.n_parameters} input parameters to {self.feature_dimensions} output features elements, \n" \
...
# ============= SAVE/LOAD model =============
    ...
    def save(self, 
        ...
        attributes = [self.W_, 
                      ...
                      self.n_layers,
                      self.architecture,
                      self.scaling_subtraction_,
                      self.scaling_division_]
    ...
    def restore(self, 
        ...
        with open(filename + ".pkl", 'rb') as f:
            ...
            self.feature_dimensions, \
            self.n_hidden, self.n_layers, self.architecture, self.scaling_subtraction_, self.scaling_division_ = pickle.load(f)
...
# ==========================================
    def train(self,
        ...
        if self.verbose:
            multiline_str = "Starting cosmoemu_tf training, \n" \
        ...
        self.parameters_mean_ = np.mean(training_parameters, axis=0)
        self.parameters_std_ = np.std(training_parameters, axis=0)
        ...
        # features scaling
        self.scaling_subtraction_ = scaling_subtraction if scaling_subtraction is not None else np.zeros(self.feature_dimensions)
        self.scaling_division_ = scaling_divison if scaling_divison is not None else np.ones(self.feature_dimensions)

        self.scaling_subtraction = tf.constant(self.scaling_subtraction_, dtype=dtype, name='subtraction')
        self.scaling_division = tf.constant(self.scaling_division_, dtype=dtype, name='division')

        training_features = (training_features-self.scaling_subtraction_)/self.scaling_division_
        
    
        # features standardisation
        self.features_mean_ = np.mean(training_features, axis=0)
        self.features_std_ = np.std(training_features, axis=0)
